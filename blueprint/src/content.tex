...
\begin{proof}
\(s(n) = \sum_{\substack{d \mid n\\ d < n}} d\) とする.
...
\(mn\) の任意の約数 \(d\) は \(d = d_1d_2 ~(d_1 \mid m, d_2 \mid n)\) と一意に表せる.
\begin{align*}
  (f \ast g)(mn) ~&=~ \sum_{d \mid mn} f(d)g(\frac{mn}{d})\\
                 ...
                 (\because ~(d_1, d_2) \text{と} (\frac{m}{d_1}, \frac{n}{d_2}) \text{はそれぞれ互いに素})\\
...
\begin{proof}
\begin{align*}
N = 2^kM_{k+1} : Perfect ~&\Leftrightarrow~ \sum_{d \mid 2^kM_{k+1}} d = 2(2^kM_{k+1}) ~\wedge~ N > 0~~~~(\because~Lemma5)\\
                         ~&\Leftrightarrow~ \sum_{d \mid 2^kM_{k+1}} d = 2^{k+1}M_{k+1} ~\wedge~ N > 0\\
                         ~&\Leftrightarrow~ \sigma(2^kM_{k+1}) = 2^{k+1}M_{k+1} ~\wedge~ N > 0~~~~~~(\because~Lemma3)\\
...
\(M_{k+1}\) と \(2^{k+1}\) は互いに素より,ある自然数 \(j\) を用いて, \(m = M_{k+1}j\) と表せる.

\(M_{k+1} \sigma(m) = 2^{k+1} M_{k+1}j\) より, \(\sigma(M_{k+1}j) = 2^{k+1}j\) が成り立つ. (\(\because~M_{k+1} \neq 0\))
\begin{align*}
\sigma(M_{k+1}j) = 2^{k+1}j ~&\Leftrightarrow~ \sum_{d \mid M_{k+1}j} d = 2^{k+1}j~~(\because~Lemma3)\\
                            ~&\Leftrightarrow~ \sum_{\substack{d \mid M_{k+1}j, d < M_{k+1}j}} d + M_{k+1}j = 2^{k+1}j\\
                            ~&\Leftrightarrow~ \sum_{\substack{d \mid M_{k+1}j, d < M_{k+1}j}} d + M_{k+1}j = (M_{k+1}+1)j~(\because~Definition8)\\
