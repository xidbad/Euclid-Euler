...
\begin{theorem}[Euclid I]

\(M_{k+1}\) が素数ならば、\(2^k*M_{k+1}\) は完全数である
...
\( \sigma(N) = 2^{n}k \) より、\( \sigma(k) = \frac{2^{n}k}{2^n - 1} = \frac{(2^{n}-1)k + k}{2^n - 1} = k + \frac{k}{2^n-1} \) が成り立つ。(\(\because \)  \(2^n - 1 \neq 0 \))
