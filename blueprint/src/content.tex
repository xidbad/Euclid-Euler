...
\begin{proof}
まず,ゼータ関数とpow関数を定義する.これらは数論的関数(Arithmetic Function)と呼ばれる.
...
これら数論的関数同士の積は次のようなディリクレ積で定義される. \(f\) を数論的関数とすると,
\begin{align*}
  (\zeta \ast f)(n) &:= \sum_{d \mid n} \zeta(d) \cdot f(\frac{n}{d})\\
  &= \sum_{i \mid n} f(i)~~(\because~\zeta(d) = 1 (d \geqq 1) \quad \text{かつ} \quad \frac{n}{d} \text{は} n \text{の約数})
\end{align*}

ここで, \(f = \mathrm{pow}_k\) とすると,
\begin{align*}
  (\zeta \ast \mathrm{pow}_k)(n) &= \sum_{d \mid n} d ^ k\\
                                 &= \sigma_k(n)
\end{align*}

乗法的関数同士のディリクレ積もまた乗法的関数であることを示す.
\(f, g\) を乗法的関数とする.互いに素な自然数 \(m, n\) に対して,
\(mn\) の約数 \(d\) は \(d = d_1d_2, (d_1 \mid m, d_2 \mid n)\) と表せる.
\begin{align*}
  (f \ast g)(mn) &= \sum_{d \mid mn} f(d) \cdot g(\frac{mn}{d})\\
                 &= \sum_{d_1d_2 \mid mn} f(d_1d_2) \cdot g(\frac{mn}{d_1d_2})\\
                 &= \sum_{d_1 \mid m} \sum_{d_2 \mid n} f(d_1)f(d_2) \cdot g(\frac{m}{d_1})g(\frac{n}{d_2})~~(\because~ (d_1, d_2) \text{は互いに素})\\
                 &= \sum_{d_1 \mid m} f(d_1) \cdot g(\frac{m}{d_1}) \cdot \sum_{d_2 \mid n} f(d_2) \cdot g(\frac{n}{d_2})\\
                 &= (f \ast g)(m) \cdot (f \ast g)(n)
\end{align*}
\(\zeta\) 関数も \(\mathrm{pow}_k\) 関数も乗法的関数であり,それらの積もまた乗法的関数であるため,
\(\sigma_k\) 関数も乗法的関数である.
\end{proof}
